	...
	return(
		<div className={styles.dropdown}>
			...
			<div className={styles.dropdown__bottom}>
				<Rows image={ChevronRightIcon}><Link className={styles.dropdown__titles} href="/">API</Link></Rows>
				<Rows click={() => window.open("mailto:short-it@7centmgmt.cf")} image={ChevronRightIcon}><a className={styles.dropdown__titles}>Contact Us</a></Rows>
				<Rows click={props.logout} image={LogoutIcon}><span className={styles.dropdown__titles}>Logout</span></Rows>
			</div>
		</div>
	)

	function Rows({ children, image, click }) {
		return(
